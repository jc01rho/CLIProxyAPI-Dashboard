    ...
    try:
        ...
        if prev_snap:
            ...
            for key in all_keys:
                ...
                if d_req > 0 or d_cost > 0:
                    ...
                    breakdown_deltas["endpoints"][endpoint]["input_tokens"] += d_in
                    breakdown_deltas["endpoints"][endpoint]["output_tokens"] += d_out
                    breakdown_deltas["endpoints"][endpoint]["reasoning_tokens"] += (
                        d_reasoning
                    )
                    breakdown_deltas["endpoints"][endpoint]["cached_tokens"] += d_cached
                    ...
                    m_data["tokens"] += d_tok
                    m_data["cost"] += d_cost
                    m_data["input_tokens"] += d_in
                    m_data["output_tokens"] += d_out
                    m_data["reasoning_tokens"] += d_reasoning
                    m_data["cached_tokens"] += d_cached
        ...
        for e, data in breakdown_deltas["endpoints"].items():
            ...
            for mName, mData in data.get("models", {}).items():
                ...
                existing["models"][mName]["input_tokens"] = existing["models"][
                    mName
                ].get("input_tokens", 0) + mData.get("input_tokens", 0)
                existing["models"][mName]["output_tokens"] = existing["models"][
                    mName
                ].get("output_tokens", 0) + mData.get("output_tokens", 0)
                existing["models"][mName]["reasoning_tokens"] = existing["models"][
                    mName
                ].get("reasoning_tokens", 0) + mData.get("reasoning_tokens", 0)
                existing["models"][mName]["cached_tokens"] = existing["models"][
                    mName
                ].get("cached_tokens", 0) + mData.get("cached_tokens", 0)
