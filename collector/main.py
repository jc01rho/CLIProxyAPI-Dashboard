...
def trigger_sync_endpoint():
    ...
    sync_thread.start()
    return jsonify(
        {"message": "Full data collection and rate limit sync process triggered."}
    ), 202
...
def reset_limit_endpoint(config_id: int):
    ...
    try:
        if not supabase:
            return jsonify(
                {"status": "error", "message": "Supabase not initialized"}
            ), 500
        ...
        config_resp = (
            supabase.table("rate_limit_configs")
        ...
        supabase.table("rate_limit_status").upsert(
            status_data, on_conflict="config_id"
        ).execute()
        ...
        logger.info(
            f"Successfully reset rate limit status AND anchor for config_id: {config_id}"
        )

        return jsonify(
            {
                "message": f"Successfully reset rate limit for config id {config_id}.",
                "new_status": {"percentage": 100, "label": label},
            }
        ), 200
...
def store_usage_data(data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        last_cost_resp = (
            supabase.table("usage_snapshots")
        ...
        last_cost_total = (
            last_cost_resp.data[0].get("cumulative_cost_usd", 0)
        ...
        snapshot_result = (
            supabase.table("usage_snapshots").insert(snapshot_data).execute()
        )
        ...
        supabase.table("usage_snapshots").update(
        ...
        existing_daily = (
            ...
            if daily_stats_resp.data
                ...
                "breakdown": {"models": {}, "endpoints": {}, "hourly": {}},
            }
        )

        # Initialize breakdown deltas
        # Include 'hourly' for time-of-day aggregation and 'failures' per model
        breakdown_deltas = {"models": {}, "endpoints": {}, "hourly": {}}

        # Get current hour for hourly aggregation
        current_hour = datetime.now(APP_TIMEZONE).hour
        hour_key = str(current_hour).zfill(2)  # "00" to "23"
        ...
        if prev_snap:
            ...
            for key in all_keys:
                ...
                if d_req > 0 or d_cost > 0:
                    ...
                    if hour_key not in breakdown_deltas["hourly"]:
                        breakdown_deltas["hourly"][hour_key] = {
                            "requests": 0,
                            "tokens": 0,
                            "cost": 0.0,
                            "models": {},
                        }
                    breakdown_deltas["hourly"][hour_key]["requests"] += d_req
                    breakdown_deltas["hourly"][hour_key]["tokens"] += d_tok
                    breakdown_deltas["hourly"][hour_key]["cost"] += d_cost

                    # Track per-model hourly data
                    if model_name not in breakdown_deltas["hourly"][hour_key]["models"]:
                        breakdown_deltas["hourly"][hour_key]["models"][model_name] = {
                            "requests": 0,
                            "tokens": 0,
                            "cost": 0.0,
                        }
            ...
            for r in model_records:
                ...
                breakdown_deltas["endpoints"][endpoint]["reasoning_tokens"] += reasoning_tok
                breakdown_deltas["endpoints"][endpoint]["cached_tokens"] += cached_tok
                ...
                m_data["reasoning_tokens"] += reasoning_tok
                m_data["cached_tokens"] += cached_tok
                ...
                breakdown_deltas["hourly"][hour_key]["requests"] += req
                breakdown_deltas["hourly"][hour_key]["tokens"] += tok
                breakdown_deltas["hourly"][hour_key]["cost"] += cost
                ...
                breakdown_deltas["hourly"][hour_key]["models"][model_name]["cost"] += (
                    cost
                )
        ...
        if "hourly" not in existing_breakdown:
            existing_breakdown["hourly"] = {}
        ...
        for m, data in breakdown_deltas["models"].items():
            ...
            existing["failures"] = existing.get("failures", 0) + data.get("failures", 0)
        ...
        for e, data in breakdown_deltas["endpoints"].items():
            ...
            for mName, mData in data.get("models", {}).items():
                ...
                existing["models"][mName]["input_tokens"] = existing["models"][mName].get("input_tokens", 0) + mData.get("input_tokens", 0)
                existing["models"][mName]["output_tokens"] = existing["models"][mName].get("output_tokens", 0) + mData.get("output_tokens", 0)
                existing["models"][mName]["reasoning_tokens"] = existing["models"][mName].get("reasoning_tokens", 0) + mData.get("reasoning_tokens", 0)
                existing["models"][mName]["cached_tokens"] = existing["models"][mName].get("cached_tokens", 0) + mData.get("cached_tokens", 0)

        # Merge Hourly data
        for h, data in breakdown_deltas["hourly"].items():
            if h not in existing_breakdown["hourly"]:
                existing_breakdown["hourly"][h] = {
                    "requests": 0,
                    "tokens": 0,
                    "cost": 0.0,
                    "input_tokens": 0,
            ...
            existing = existing_breakdown["hourly"][h]
            existing["requests"] += data["requests"]
            existing["tokens"] += data["tokens"]
            existing["cost"] += data["cost"]
            existing["input_tokens"] = existing.get("input_tokens", 0) + data.get("input_tokens", 0)
            ...
            # Merge nested models within hour
            if "models" not in existing:
                existing["models"] = {}
            for mName, mData in data.get("models", {}).items():
                if mName not in existing["models"]:
        ...
        final_tokens = (
            ...
            else (existing_daily.get("total_tokens", 0) + inc_tokens)
        )
        ...
        final_requests = (
            ...
            else (existing_daily.get("total_requests", 0) + inc_requests)
        )
